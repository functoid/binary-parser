...
Parser.prototype.getCode = function() {
    ...
    this.closures = this.generate(ctx).closures;
...
Parser.prototype.compile = function() {
    this.compiled = new Function('buffer', 'callback', 'constructorFn', 'closures', this.getCode());
};
...
Parser.prototype.parse = function(buffer, callback) {
    ...
    return this.compiled(buffer, callback, this.constructorFn, this.closures);
};
...
Parser.prototype.generateBit = function(ctx) {
    ...
    if ((sum % 8) === 0 || !this.next || (this.next && ['Bit', 'Nest'].indexOf(this.next.type) < 0)) {
...
Parser.prototype.generateBuffer = function(ctx) {
    ...
    } else if (typeof this.options.readUntil === 'number' && this.options.readUntil < 0) {
        ctx.pushCode('{0} = buffer.slice(offset, buffer.length - {1});',
            ctx.generateVariable(this.varName),
            -this.options.readUntil
            );
        ctx.pushCode('offset = buffer.length - {0};', -this.options.readUntil);
    } else if (this.options.readUntil instanceof Buffer) {
        var index = ctx.generateTmpVariable();
        var varName = ctx.generateVariable(this.varName);
        ctx.pushCode('var {0} = buffer.indexOf(new Buffer("{1}", "hex"), offset);',
            index, this.options.readUntil.toString('hex')
            );
        ctx.pushCode('if ({0} >= 0) {', index);
        ctx.pushCode('{0} = buffer.slice(offset, {1});', varName, index);
        ctx.pushCode('offset = {0} + {1};', index, this.options.readUntil.length);
        ctx.pushCode('} else {');
        ctx.pushCode('{0} = buffer.slice(offset, buffer.length);', varName);
        ctx.pushCode('}');
    } else {
...
Parser.prototype.generateFormatter = function(ctx, varName, formatter) {
    if (typeof formatter === 'function') {
        if (!this.options.capture) {
            ...
            var index = ctx.pushClosure(formatter);
            ctx.pushCode('{0} = (closures[{1}]).call(this, {0});', varName, index);
        }
    }
};
